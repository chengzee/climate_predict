...
import csv 
import time
for number in range(len(12)):
    rawdata = pd.read_csv("sensorNode{}.csv".format(number+1)) # 讀資料進來
    rawdata_array = np.array(rawdata) # 轉為矩陣
    print("sensorNode{}".format(number+1))
    total_LossNumber = 0
    ...
    with open("paddeddata{}.csv".format(number+1), 'w', newline='') as thecsvfile:
